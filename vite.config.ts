...
export default defineConfig({
  ...
  server: {
    ...
    proxy: {
      // Proxy for checking devnet availability (bypasses CORS in localhost)
      '/api/devnet-status': {
        target: 'https://next.devnet.aztec-labs.com',
        changeOrigin: true,
        rewrite: (path) => path.replace(/^\/api\/devnet-status/, '/status'),
      },
    },
  },
  preview: {
