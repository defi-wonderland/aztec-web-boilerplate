...
/**
 * Generate proxy path for remote network status checks.
 * Only devnet has a configured server-side proxy route.
 */
const getProxyPath = (
  network: AztecNetwork,
  nodeUrl: string
): string | undefined => {
  if (isLocalhost(nodeUrl)) return undefined;
  if (network === 'devnet') return '/api/devnet-status';
  return undefined;
};
  ...
  const results = await Promise.all(
    entries.map(async ([network, config]) => {
      const available = await checkNetworkAvailable(
        config.nodeUrl,
        getProxyPath(network, config.nodeUrl)
      );
