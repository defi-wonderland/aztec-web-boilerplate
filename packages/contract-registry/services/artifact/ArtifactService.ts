...
import type { ContractConfigMap, ArtifactSourceConfig } from '../../core/types';
...
export class ArtifactService {
  ...
  async loadArtifacts(
    ...
    const results = await Promise.all(
      contractNames.map((name) => {
        ...
        return this.loadSingleArtifact(name, sources, classId);
      })
    );
  ...
  /**
   * Load a single contract artifact using an ordered fallback chain of sources.
   * Returns on first success. All sources exhausted without success is an error.
   */
  async loadSingleArtifact(
    contractName: string,
