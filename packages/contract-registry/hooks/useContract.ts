...
import { useContractRegistryStore, useContractRegistryStatus } from '../store';
import type {
  ContractConfigMap,
  ContractStatus,
  UseContractReturn,
...
export function useContract<T extends string>(
  name: string extends T ? T : never
): UseContractReturn<unknown>;
export function useContract(name: string): UseContractReturn<unknown> {
  type TContract = unknown;
  const { wallet: walletAdapter, contracts } = useRegistryContext();
  ...
  const isBrowserWallet = walletAdapter.isBrowserWallet;
  const wallet = walletAdapter.getWallet();
  ...
  const getContractDefinition = useCallback(() => {
    return (contracts as ContractConfigMap)[name] ?? null;
  }, [name, contracts]);
  ...
  const createExternalWalletContractProxy = useCallback((): TContract => {
    ...
    const contractAddress = AztecAddress.fromString(
      definition.address(walletAdapter.currentConfig)
    );
    ...
    walletAdapter.account,
    walletAdapter.currentConfig,
    ...
    const updateState = async () => {
      ...
      if (currentStatus === 'ready' && wallet && !hasCreatedContract.current) {
        ...
        if (instance && definition && resolvedArtifact) {
          ...
          try {
            const callableContract = await walletAdapter.queuePxeCall(
              async () =>
  ...
  // Trigger lazy registration only for contracts that haven't been attempted yet.
  // Skip if already registered, in-progress, or failed (no auto-retry on error).
  ...
  useEffect(() => {
    ...
    if (currentStatus === 'idle' && !registrationAttemptedRef.current) {
  ...
  useEffect(() => {
    ...
    walletAdapter.account,
    createExternalWalletContractProxy,
    isBrowserWallet,
  ]);
